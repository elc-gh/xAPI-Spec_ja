...
に注意。LRS は他の LRS やレポーティングシステムと通信をすることができる。
...
大まかにいうと、文書が技術的に見えたり、要求仕様のようである場合は、そのように解釈し
...
__コミュニティプロファイル__: 実践コミュニティのユースケース、予約語、実装
仕様、事前定義された実例（例：レシピ）などの、xAPIを用いた特定の学習アクティ
ビティをどのようにとらえるかをまとめた独自の資料群

<a name="def-controlled-vocabulary" />

__予約語__: 実践コミュニティにより構築される、制限され、合意された用語のリ
スト。予約語の目的は、xAPIデータの意味の表現、保管、および取り出しに一貫性
を持たせることである。リストが管理される理由は、用語が特定の課題領域やドメ
インにおいて利用されることが想定されるからである。リスト自体の保守も管理さ
れるべきである。用語のリストは拡張されていくが、実践コミュニティの方針に従
う。
...
<a name="def-iri" /></a>
__IRI (Internationalized Resource Identifiers)__: IRL でありうる一意
な識別子。xAPI では、全ての IRI はスキームを含む完全な絶対 IRI となるべ
きである。相対 IRI は使われるべきではない。 IRL は IRL を作る者がコント
ロールするドメインの中で、定義されるべきである。
...
__プロファイル__: 一般的に「アプリケーションに対して何らかの意味をもつ名前と文書の
組」によってエージェントやアクティビティの情報を保持する構成体である。
...
<table>
    ...
    <td>ステートメントの目的語となる アクティビティ、エージェント、または別のステートメント。"I Did This" の "This" に相当。</td><td>必須</td></tr>
...
###### ステートメントの不変性とその例外
ステートメントは不変（変更できない）である。以下は、例外または本ルールによってカバー
されない領域である。

* LRS内のプロセス中の、可能性のあるまたは必要なプロパティの割当（"id", "authority",
"stored", "timestamp", "version")

* ステートメントにより参照されるアクティビティ。ステートメント中で参照されるアクティ
ビティの内容は、ステートメントそのものの一部とは見なされない。これは、参照されるアク
ティビティが変更された時、ステートメントのディープシリアライゼーションによって生成さ
れるJSONも変更されることを意味する（ステートメント API の "format" パラメータ参照）。

* ステートメントによって参照される動詞。動詞のディスプレイプロパティは、ステートメン
トその物の一部とは見なされない（ステートメント API の"format"要素を参照）

* タイムスタンプデータのシリアライゼーション。これは、不変のステートメントの一部とは
見なされない。たとえば、タイムスタンプとステートメントに記録された要素は、参照される
時間が影響を与えない限り、記録されている場所と異なるタイムゾーンで返答されてもよい。
詳細は、[4.1.7 Timestamp](#timestamp)及び[4.1.8 Stored](#stored)を参照。

* 順序づけされていないリストのシリアライゼーション。グループ中のエージェントのリスト
は、順序づけされたリストとは見なされず、LRSがエージェントのリストを任意の順序で返答し
...
<table border ="1">
    ...
    <td>このグループのメンバー。これは順序を指定しないリストである。</td>
    <td>必須</td></tr>
</table>
...
* ステートメントを返答するLRSは、グループ中のメンバーのリストをどのような順序で返答しても良い。
...
<table>
    ...
    <tr>
        <td>id</td>
        <td>IRI</td>
        <td>動詞の定義を示す。それぞれの動詞の定義は、単語ではなく動詞の意味に対応する。
        IRI は人間が読める形式で、動詞の意味を含むべきである。
        </td>
        <td>必須</td>
    </tr>
    <tr>
...
* ID に含まれる IRI は、ステートメントを参照する人が（文法的に）類似の動詞から、対象
　の動詞を区別するに十分な意味情報を提供する、人間が読むことのできる部分を含むべきである。
* ひとつの動詞 IRI は、複数の意味を示すために使われてはならない。

###### 動詞表示 AP の必要条件
* display プロパティは全てのステートメントにおいて使われるべきである。
* display プロパティは、動詞 IRI によって定義されている意味を表すために使われなければならない。

###### 動詞表示 LRS の必要条件
以下に示す必要条件は API を使って LRS によって返される display プロパティに関係するものである。

* format として "exact" を指定してステートメントが呼ばれた場合、LRS  は display プロパティ
　をステートメントに示された（または省略された）通りに返さなければならない。
* format として "ids" を指定してステートメントが呼ばれた場合、LRS は display プロパティを
　含むべきではない。
* format として "canonical" を指定してステートメントが呼ばれた場合、LRS は動詞の正規化さ
　れた display を返すべきである。
* LRS はその正規化された display を受信するステートメントに含まれる動詞 display プロ
　パティ、[section 5.4 Identifier metadata](#miscmeta) に示されるメタデータに含まれる name プロパティ、またはその他の場所にて定義されている動詞 display に基づいて決定しても良い。

###### 動詞 display クライアントの必要条件
以下に示す必要条件は、LRS またはその他のシステムによって利用者に表示される display
プロパティに関連するものである。

* display プロパティは動詞の意味をまげるために利用してはならない。
* ステートメントを受信するシステムは、display プロパティを、ステートメントのいかなる
　意味をも推論するために利用してはならない。
* ステートメントを受信するシステムは、display プロパティを、人に対して表示する以外の
　いかなる用途にも利用してはならない。

ステートメントの集約やカテゴライズのために display プロパティを利用することは、この
必要条件に対する違反となる。

* ユーザインターフェイスにステートメントの動詞を表示するシステムは、ステートメントに
　含まれる display プロパティ、[section 5.4 Identifier metadata](#miscmeta) に示されるメタデータに含まれる name プロパティ、またはその他の場所にて定義されている
動詞 display のいずれかを表示するために選択してもよい。
* ステートメントの動詞を表示するシステムは、動詞の意味と異なる単語を表示してはならな
いが、読みやすさのために言い回しや時制を変更して表示しても良い。
...
など、文脈に応じて異なる意味を持ち得る。この例においては、IRI はそれらの特定の一つの意味を示す。
...
このプロパティの値として提供されるオブジェクトは "objectType" プロパティを持つべきで
...
らのアクティビティに対する妥当性は疑問視されることになる。これは、LRS
が、同じアクティビティIDを２つの異なるアクティビティへの参照として扱う
ことが、意図したとしても、出来ないことを意味する。すなわち、他のシステ
ムとコンフリクトが生じたとき、その意図をくみ取ることはできない。
...
<table>
    ...
    <tr>
        <td>name</td>
        <td><a href="#misclangmap">言語マップ</a></td>
        <td>可読／可視化したアクティビティの名称。動詞では、ステートメントにおける display プロパティに相当する。</td>
        <td>推奨</td>
    </tr>
    <tr>
        <td>description</td>
...
* LRS は、アクティビティの定義に関する小修正を受け入れてもよい。例えば、スペルの訂正である。但し、正解に対する変更を受け入れてはならない。
...
<table>
    ...
    <tr>
        <td>matching</td>
		<td>学習者がある項目群（ソースセット）と別の項目群（ターゲットセット）の対応付を要求されるインタラクション。項目は、必ずしも組になるとは限らず、ソースセット中の複数あるいは０個の項目が、ターゲットと組になる、あるいはその逆ということが起こりうる。</td>
    </tr>
    <tr>
        <td>performance</td>
    ...
    <tr>
        <td>sequencing</td>
        <td>学習者がある要素群の順序を並べ替えることを要求するインタラクション。</td>
    </tr>
    <tr>
        <td>likert</td>
...
###### 説明
サブステートメントは、ステートメントの一部となるステートメントのようなものであるが、（ステートメントと異なり）実際に起こったイベントに対応するものではない。たとえば、生徒を評価する際に教師が求める行動（実際に生徒がその行動を起こすことなしに）といった、将来起こりうるステートメントのパターンを記述することに使うことが出来る。
...
###### 説明
エージェントによって達成された、類別されたアクティビティの結果を表わすオプションの数字プロパティ
...
* スコアオブジェクトは、進捗もしくは完了に関連するスコアのために使用すべきではない。代わりに
<a href="https://github.com/adlnet/xAPI-Spec/blob/master/xAPI.md#miscext">拡張</a>の
利用を検討すること。 
...
###### 説明
文脈依存の情報をステートメントに付加するための任意のプロパティ。全てのプロパティは任意である。

###### 背景
context プロパティは文脈依存の情報をステートメントに付加する機会を提供する。
...
義である。カテゴリは、カテゴリ化だけでなく、xAPIの挙動に関する<a href="#def-community-profile">コミュニティプロファイル</a>を示すために利用されるべきである。

例えば、アンナは生物学の試験に挑戦する。そしてステート
メントは(訳注: AICC の) CMI-5 プロファイルを用いて記録される。そのステートメ
ントのアクティビティは試験を参照し、そのカテゴリは CMI-5 プロファイルとなる。

4. __Other__: 他のプロパティのいずれにも合わない context アクティビティ。例え
...
[Stored](#storead)で用いるものと同じ値を割り当てる。

ステートメント中のタイムスタンプは Stored (ステートメントが記録された時刻) 
と異なる場合がある。たとえば、経験が発生してから LRS がステートメントを受
信するまでの間に、遅延が発生することがある。
...
* タイプスタンププロパティは、指定されない場合、LRSによって、[Stored](#stored)
の値に設定されるべきである。
* timestamp はタイムゾーンを含むべきである。
* タイムスタンプにタイムゾーンが含まれる場合、LRSは参照される時間が変更されない限り、もとも
とのタイムゾーンと異なるタイムゾーンを利用したタイムスタンプを返答しても良い。LRSは、UTCタイ
ムゾーンを用いたタイムスタンプを返答すべきである。
* タイムスタンプは、経験の期間中、始点や終点に限らず、任意の点を指定して良い。
* timestamp は、秒を少なくとも3桁の精度で切り捨てられるか、あるいは丸められてもよい(ミリ秒の精度は保たれなければならない)。
* APは、ステートメント中のタイムスタンプとして、未来の時間を指定してはならない。
* LRSは、時計のエラーに基づく問題発生を防ぐために、未来の時間が指定されたタイムスタンプを拒絶しなければならない。
* サブステートメントは、将来の時間を示すタイムスタンプを持って良い。
...
###### 詳細
stored プロパティは、ステートメントが記録されたときの正確な時刻である。LRSは、ステ
...
* stored プロパティにタイムゾーンが含まれる場合、LRSは参照される時間が変更されない限り、もとも
とのタイムゾーンと異なるタイムゾーンを利用した stored プロパティを返答しても良い。LRSは、UTCタ
イムゾーンを用いた stored プロパティを返答すべきである。
...
* OAuth consumer を示すエージェントは、「アカウント名」のプロパティの値として consumer キーを用いなければならない。
...
###### 背景
場合によっては、添付文書が学習記録の論理的に重要な部分となる。航空管制との通信シ
ミュレーション、エッセイ、ビデオなどを考えてみてほしい。添付文書の他の例としては、
経験の結果として与えられた修了証書（の画像）がある。これらの添付文書をLRS に記録
したり LRS から読み出したりする方法があることは有益である。サブステートメントに
文書を添付したい場合、添付文書をステートメントアタッチメントオブジェクトとして、
通常のステートメントにペイロードとして含めることを強く推奨する。
...
* ステートメントに対して、本仕様書にて明示的に許可されていない限り、追加のプロパティ
を付け加えるべきではない。

注記：LRSは、追加のプロパティを含むステートメントを拒絶することが推奨される。ステー
トメント中の追加のプロパティは、そのステートメントが互換性を持たないLRSが存在しうる
ことを意味する。
...
* 以下の場合、LRS はステートメントを拒否しなければならない。
    ...
    * キーの大文字小文字の別が、本仕様書における大文字小文字の別に合致しない場合。
	* 列挙型で与えられる値の大文字小文字の別が、本仕様書における列挙型の値に合致しない
	　場合。
	* キーまたは値が、本仕様書において許可されていない場合。
	* キーが１つのオブジェクト中に複数現れる場合。
...
* 追加のプロパティは、本仕様書において明示的に許可された場合を除き、ステートメントや他のオブジェクトに追加されるべきではなく、LRSはそのような追加のプロパティを含むステートメントを拒絶すべきである。
...
* 他のステートメントを無効化するステートメントを発行するときは、無効化ステートメントの目的語は "objectType" プロパティを "StatementRef" に設定しなければならない
* 他のステートメントを無効化するステートメントを発行するとき、無効化ステートメントの目的語では、無効化すべきステートメントの ID を ID プロパティで指定しなければならない。
...
###### 説明
xAPIは、アクティビティプロバイダに対して、アクティビティ、エージェント、
もしくはその両方のドキュメント形式による任意のデータを記録する機能を提
供する。
...
識別子に関する追加の情報をステートメント含め、識別子 IRI によって指定される場所にて保管
することができる。ステートメント中にメタデータを含めることは、IRI に関するメタデータを、
それを解決することなしに表現することを可能にする。IRI の場所にてメタデータを保管すること
は、IRI の所有者が、その IRI の標準的なメタデータを定義することを可能にする。
...
アクティビティ ID についての保管されたメタデータの構造については、<a href="#activity">
アクティビティ定義オブジェクト</a> を参照のこと。

それ以外の識別子に関する保管されたメタデータの構造については、以下のフォーマットを参照
のこと。
...
<table>
        ...
        <td>name</td>
            <td><a href="#misclangmap">Language Map</a></td>
            <td>人が読める／見られる 名前。動詞においては、これはステートメント中の display 要素に相当する。</td>
            <td>任意</td>
       </tr>
       <tr>
        <td>description</td>
...
保管されたメタデータは、上に示す JSON オブジェクトを含む文書からなる。保管されたメタデータが提供される場合、それは識別子
が示す情報についての標準的な情報源となる。アクティビティ ID 以外に関し、すべてのタ
イプの IRI 識別子に、確立され広く認められている識別子を利用することをアクティビティ
プロバイダに対して推奨する。
...
* LRS が PUT または POST リクエストに応答する際には、作成または変更したエンティティに対して、ETag HTTP ヘッダを添付すべきである。
...
* 利用者が次のことをすべきであることを説明するレスポンスを返さなければならない。
    - リソースの現在の状態をチェックする。
    - 競合を解消するために現在の ETag を設定した "If-Match" ヘッダをセットする。
* リソースを変更してはならない。
...
文字列やフォームパラメータとみなして引き渡されなければならない。
...
この仕様書は、LRS が、特定の条件において、リクエストの受付・拒絶、レスポンスの応答及びその他
の挙動を行う要求を定義する。LRS がリクエストを拒絶する必要がある場合、要求の一部として、適切
なエラーコードを示す。

これらのいかなる要求も、この仕様書のコープの範囲外における条件に基づいて、要求を拒絶し、異な
る方式で応答するといったように、LRS を設定しても良いという考えと矛盾する物ではない。

そのような条件の一つは、パーミッションである。

たとえば、LRS は、条件が特定のエージェントのみに発行されるように、特定の組合せの条件に対して
パーミッションを割り当てる。また、エージェントに関係のない条件を用いるステートメントは拒否を
することができる。LRS によって与えることの出来るパーミッションは、[6.4.2](#oauthscope)に
示す、推奨されるOAuthのスコープ認証値リストを除いて、本仕様書のスコープ外である。

パーミッションはGETリクエストに対してLRSが返す返答にも影響を与える。たとえば、条件の組は、特
定のアクタについてのステートメントを参照する許可を持つかもしれず、その場合、LRSはそのアクタ
に関連しないステートメントをフィルタする。詳細は、[Section 7.2.3 GET Statements](#stmtapiget)
を参照のこと。

本仕様書により明示的に許可される場合、指定される条件は、要求を取り扱うLRSの挙動にも影響を与
える。たとえば、LRSは通常、ステートメントのAuthority要素を上書きするが、ステートメント発行
に使われた証明書に関して強い信頼関係がある場合、受信した権限を受け入れても良い。詳細は
[Section 4.1.9 Authority](#authority)を参照のこと。

LRSによって設定されるパーミッションは、技術的には適合しているLRSを、適合テストに失格させる
原因となる可能性がある。これは、テスト手段が発行する要求がパーミッションによって拒絶される
場合に起こりえる。この理由から、パーミッション制限が適合性テストの結果に影響を与えないように、
LRSは設定可能である必要が有り、テストに用いられる証明書は、適切なパーミッションを与えられて
いる必要がある。

その他としては、送信される要求が、LRSによって設定されるサイズの制約を超える場合が考えられる。
LRSがいかなる場合にも、どのようなサイズの要求を受けとると期待することは妥当ではない。LRSは
必要に応じてどのようなサイズ制限を選択することも出来るが、適合性テストの時には、サイズ制限を
回避できるように設定可能でなければならない。もちろん、ハードウェアなどの制約のために、適合性
テストにおいても、何らかのサイズ制限が存在しうるが、その制限はテストの実行の妨げにならないよ
うに、妥当に高いものであることが期待される。

LRSは不正が疑われる場合は、要求を拒否したり、証明書を無効にしたりすることができる。たとえば、
短い時間の間に、大量の要求が送付されるなどの場合である。適合性テストの期間中の要求のレートが、
レートの制限にかかることがないように、制限は妥当に高いものであることが期待される。


###### 詳細
下記のリストは、APIの各種メソッドによって返されるHTTPエラーコードに関する一般的な指針を示す。
...
* ```413 Request Entity Too Large``` - LRSが、許可する最大値よりも大きい（あるいは、要求
に含まれる添付書類のサイズが大きい）ことを理由に、ステートメントもしくはドキュメントを拒否し
たことを示す。

* ```429 Too Many Requests``` - 一定期間内にクライアントより多量の要求を受信したために、
LRSが要求を拒否したことを示す。
...
* LRSは、エラーのフォーマットを決めるために、[RFC 7231](http://tools.ietf.org/html/rfc7231#section-5.3)に示されるコンテンツネゴシエーションを利用すべきである。

* LRSは、（コンテンツネゴシエーションを用いる）エラーに対して、プレーンテキスト、HTML、およびJSONレスポンスを許可すべきである。

* APはコンテンツネゴシエーションを可能にするために、要求にAcceptヘッダーをつけて送信すべきである。

* LRS はレスポンス中において、エラーの原因を説明するメッセージを返すべきである。

* LRSは、本仕様書に基づいて意図した文脈においてLRSが認識できないパラメータを利用するAPIに
対する要求について、ステータスとして```HTTP 400 Bad Request```で拒絶しなければならない。
（__注意：__ LRSは本仕様書にないパラメータに対しての挙動を認識しても良い）

* LRSは、（訳注：おそらく脱字）の場合を除いて、本仕様書に記述されるパラメータにマッチする
パラメータを利用するAPIに対する要求について、ステータスとして```HTTP 400 Bad Request```で拒絶しなければならない。

* LRSは、バッチに含まれるステートメントが拒絶される場合、バッチ全体を拒絶しなければならな
い。

* LRSは、要求に対応する証明書が、その要求を実行するパーミッションを持たない場合、ステータ
スとして```HTTP 403 Forbidden```で拒絶しなければならない。

* LRSは、許可された最大値より大きなサイズの添付書類、ステートメントまたは文書についての要
求を、ステータスとして```HTTP 413 Request Entity Too Large```で拒絶しなければならない。

* LRSは、添付書類、ステートメント及び文書のサイズ制限を決めることが出来、この制限を、たと
えば権限毎などの、様々な基準に基づいて変更させても良い。

* LRSは、一定の時間内に、あるクライアントや証明書から、大量の要求が送られたために、要求を
拒絶する場合、ステータスとして```429 Too Many Requests```で拒絶しなければならない。

* LRSは、要求受信レートの制限を任意に設定することが出来、この制限を、たとえば権限毎などの、
様々な基準に基づいて変更させても良い。

以下の要求は、適合性テストのために存在し、LRSによって設定される各種の制限や適用されるパー
ミッションが、適合性テストの障害にならないことを目的とする。

* LRSは、特定の証明書の組からの要求を、パーミッションを前提に拒絶しないように設定可能であ
るべきである。この証明書の組は、適合性テストに利用されるべきで、ライブ環境においては、削除
または無効化されてよい。

* LRSは、妥当なサイズ（上記参照）の添付資料、ステートメントまたは文書を受け入れられるよう
に、設定可能でなければならない。

* LRSは、妥当な送信レートでの要求を受け入れられるように、設定可能でなければならない。
...
与えられた ID の単一のステートメントを格納する。単一のステートメントの格納にはPOSTも利用可能である。
...
* アクティビティプロバイダはステートメント"id"プロパティを含むステートメントを、PUTではなくPOSTすべきである。
* ステートメントをPUTする場合、ステートメントの"id"プロパティが利用されるべきである。
...
大量のステートメント群を生成するシステムのための代替策は AP上の API の LRS 
側を提供することである。そして定期的にステートメント群を更新（もしくは新規）する
よう LRS からその API を照会する。これは大量のデータを LRS に提供するようなシステ
ムにおいてのみ現実的なオプションといえる。
...
される。ステートメント ID もしくは voidedStatement id がパラメータとして指定され
...
<table>
    ...
    <tr><td>registration</td><td>UUID</td><td> </td>
        <td>フィルタし、指定した registration id に一致するステートメントを返す。あるアクティビティに割り当てられるあるアクタに対して、一意の登録 ID が割り当てられることが多いが、それを前提とできないことに注意が必要である。特定のアクタもしくはアクティビティのためのステートメントのみが求められる場合には、それらのパラメータもあわせて指定されなければならない。</td><td>任意</td>
    </tr>
    <tr>
...
* LRSは、使用される証明書に関連するパーミッションに基づいたクエリーフィルタ条件を適用
しても良い。
...
__注釈:__コンテキストの中のステートメントプロパティの値として用いられる StatementRefs は、ステート
メントのフィルタに対して影響を与えない。
...
* 表示のための言語マップオブジェクトを含む動詞オブジェクト。LRSはこのマップの中からただ１つの言語を返答すべきである。
...
* ここに示されない追加のプロパティは、このオブジェクトに追加されるべきでは
なく、それぞれのプロパティはただ一度だけ指定されなければならない。
...
と考えられている。）
* 追加のプロパティは拡張の外側でこのオブジェクトに追加されてはならず、それぞれのプロパティは一度だけ指定されなければならない。
...
__注意事項__: Internet Explorer 10 より低いバージョンは、HTTP と HTTPS 間でクロスド
メインリクエストをサポートしていない。これは IE9 とそれ以下を意味しており、 LRS
...
* コンテキストにおけるステートメントプロパティがセットされていた場合、ステートメントから削除すること。
